...
const EditEvent = () => {
  ...
  const [initialOptionNextValues, setInitialOptionNextValues] = useState([
    null,
    null,
    null,
  ]);
  ...
  const handleSaveOption = (index) => {
    ...
    if (
      ...
      initialOptionNextValues[index] !== null
    ) {
      // Add logic to update existing option
      const existingOptionIndex = newEventOptions.findIndex(
        (e) => e.index === index
      );
  ...
  const handleEditEvent = async (e) => {
    ...
    for (let i = 0; i < newEventOptions.length; i++) {
      ...
      if (
        option.next === null ||
        !allEvents.some((event) => event._id === option.next)
      ) {
        //define new event
        const newEvent = {
          name: option.name,
        ...
        if (!createdEvent) {
          console.error(
            "An error occurred when creating the event:",
            createdEvent
          );
          ...
          console.log(
            "What is the new story looks like after new event:",
            story
          );
          ...
          {options.map((option, index) =>
            option ? (
              <div key={index}>
                ...
                <p>Option Text:</p>
                <textarea
                  className="CustomOptionTextArea"
                  value={option.text}
                  onChange={(e) =>
                    handleOptionChange(index, {
                      ...option,
              ...
              <button
                key={index}
                onClick={() =>
                  handleOptionChange(index, { name: "", text: "" })
                }
              >
                Add Option {index + 1}
              </button>
