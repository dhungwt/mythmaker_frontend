...
function IndividualStoryPage() {
  ...
  useEffect(() => {
    ...
    if (!story || story?._id !== id) fetchStory();
  }, []);
  ...
  const addNextEvent = (option) => {
    ...
    if (newEvent) setDisplayEvent([...displayEvent, newEvent]);
    else setTheEnd(true);
  };
  ...
  const playerChoice = () => {
    ...
    if (tempEvent) {
      ...
      return (
        <div>
          {tempEvent.option1 && (
            <button onClick={() => addNextEvent(tempEvent.option1)}>
              {tempEvent.option1.text || "..."}
            </button>
          )}
          {tempEvent.option2 && (
            <button onClick={() => addNextEvent(tempEvent?.option2)}>
              {tempEvent.option2?.text || "..."}
            </button>
          )}
          {tempEvent.option3 && (
            <button onClick={() => addNextEvent(tempEvent?.option3)}>
              {tempEvent.option3?.text || "..."}
            </button>
          )}
        </div>
      );
  ...
  return (
    <div>
      ...
      )}
      {
        ...
        theEnd ? finishScene() : playerChoice()
      }
    </div>
  );
}
